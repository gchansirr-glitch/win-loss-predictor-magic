// All VIP / user data now runs through the public (anon / publishable) Supabase
// client and is gated by RLS — there is no service_role secret in this app.
// The admin allow-list only controls what the UI exposes; the database policies
// are what actually permit the reads and writes.
...
export async function listUsers(adminId: string) {
  if (!isAdmin(adminId)) throw new Error("Forbidden");
  const { data: rows } = await supabase
    .from("app_users")
    .select("*")
    .order("last_seen_at", { ascending: false })
    .limit(200);
  return { users: (rows ?? []) as AppUserRow[] };
}

export async function setVip(data: {
  adminId: string;
  telegramId: string;
